...
def _cli():  # mirror previous main block
    if len(sys.argv) not in (5,6):
        print("Usage: python anomaly_cv.py <baseline.jpg> <maintenance.jpg> <overlay.png> <report.json> [slider_percent]")
        sys.exit(1)
    _, bpath, mpath, opath, jpath, *rest = sys.argv
    slider_arg = float(rest[0]) if rest else None
    rep = detect_anomalies(bpath, mpath, opath, jpath, slider_percent=slider_arg)
